...
export const withCIDFromFiles = async (request: IRequest, env: Env) => {
...
export const withCIDFromPinning = async (request: IRequest, env: Env) => {
...
export const withCIDUnpinned = async (request: IRequest, env: Env) => {
...
export const handleGetFileFromCID = async (request: IRequest, env: Env) => {
  let { params: { cid } } = request
  return Response.redirect(`${env.IPFS_GATEWAY_ENDPOINT}/ipfs/${cid}`, 307)
}

export const handlePutFileCID = async (request: IRequest, env: Env) => {
...
export const handleDeleteFile = (scope: 'files' | 'large_files') => async (request: IRequest, env: Env) => {
  const { params: { address, filename } } = request
